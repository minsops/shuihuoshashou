...
- Manual WebSocket probes validate optional timing/confidence metadata and report recoverable
  errors for invalid values.
- Unknown WebSocket event types return recoverable errors instead of being silently ignored.
...
- Scoring requires AIGC results to cover every transcript turn and rejects duplicate or unknown
  AIGC turn ids before producing a score; scoring/report build request schemas reject empty AIGC
...
- WebSocket binary frames return a recoverable `error` event without closing the session.
- WebSocket invalid JSON text frames return a recoverable `error` event without closing the session.
