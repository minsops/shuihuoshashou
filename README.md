...
`error` event for the same reason. Invalid JSON text frames and non-object WebSocket JSON payloads
are rejected with an `error` event while keeping the session open for later valid events. Premature
`end` events that fail state guards also return an `error` event without closing the session. If
audio metadata is provided, the gateway accepts only PCM/Opus-style formats,
`sample_rate_hz=16000`, and `channels=1`; unsupported values are rejected before reaching ASR.
